...
peak_folder = '/Users/brian_z/Desktop/Research/ET_hBN_fitting/Processing files using matlab/peak_data/'
...
wns=readmatrix('/Users/brian_z/Desktop/Research/ET_hBN_fitting/Processing files using matlab/DataFiles/221121_junhe/file_wns_4.csv');
% wns = wns(4:19,:)
...
setnum=4; %and only the one labelled as _1
...
% hold on;
for a=1:length(wns)
    filename=sprintf("%g/hBN_ET_50K_%g_S4_fwd_4.csv",wns(a),wns(a))
...
for b=1:length(wns)
    ...
    temp = vertcat(transpose(xs_shift),line_normed)
    path = strcat(line_folder,sprintf("hBN_ET_50K_%g_S4_fwd_4.csv",wns(b)))
    writematrix(temp, path)
...
%     offset = 1.6e-07
%     for d=1:4
%         peak = xpk(all_indices(c,d))
%         I = find(x<(peak+offset) & x>(peak-offset))
%         a = transpose(x(I))
%         b = transpose(y(I))
%         path = strcat(peak_folder,sprintf("p%g_hBN_ET_50K_%g_S4_fwd_1.csv",d,wns(c)))
%         temp = vertcat(a,b)
%         writematrix(temp, path)
%     end
% %     peak_1 = xpk(all_indices(c,1))
% %     peak_2 = xpk(all_indices(c,2))
% %     peak_3 = xpk(all_indices(c,3))
% %     peak_4 = xpk(all_indices(c,4))
% %     I_1 = find(x<(peak_1+offset) & x>(peak_1-offset))
% %     peakdata(1,:) = x(I_1)
% %     peakdata(2,:) = transpose(y(I_1))
% %     I_2 = find(x<(peak_2+offset) & x>(peak_2-offset))
% %     peakdata(3,:) = x(I_2)
% %     peakdata(4,:) = transpose(y(I_2))
% %     I_3 = find(x<(peak_3+offset) & x>(peak_3-offset))
% %     peakdata(5,:) = x(I_3)
% %     peakdata(6,:) = transpose(y(I_3))
% %     I_4 = find(x<(peak_4+offset) & x>(peak_4-offset))
% %     peakdata(7,:) = x(I_4)
% %     peakdata(8,:) = transpose(y(I_4))
% %     plot(x_tbp,y_tbp+0.3*c)
% %     result(c) = xpk(ind)
